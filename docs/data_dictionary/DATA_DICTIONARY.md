...
| `macro_news_articles` | Ingestion Table | 1 article | `article_id` | DuckDB Table |
| `macro_economic_releases` | Ingestion Table | 1 scheduled event | `release_id` | DuckDB Table |
| `daily_narrative_intelligence` | Macro Mart | 1 UTC day | `intelligence_date` | DuckDB Table |
| `mart_macro_narrative_daily` | Conformed Mart | 1 UTC day | `trade_date_utc` | DuckDB SQL View |
| `user_market_intelligence` | Ingestion Table | 1 user intelligence note | `intelligence_id` | DuckDB Table |
...
## 14. Phase 17: Agentic Investment Committee & User Intelligence Tables & Marts

### 14.1 user_market_intelligence

#### Description
Stores user-submitted market intelligence, qualitative hypotheses, research notes, and scraped article contents with sentiment polarity and pillar classifications.

- **Layer**: User Intelligence Ingestion Layer
- **Physical Location**: `data/state/platform.duckdb`
- **Primary Key**: `intelligence_id`

| Column Name | Data Type | Nullable | Description & Business Rules |
| :--- | :--- | :--- | :--- |
| `intelligence_id` | `VARCHAR` | No | Deterministic SHA-256 hash or UUID identifier (`intel_...`). Primary key. |
| `source_url` | `VARCHAR` | Yes | Canonical URL of external research note or upstream news article. |
| `title` | `VARCHAR` | No | Headline summary or document title. |
| `user_thesis` | `VARCHAR` | No | Qualitative hypothesis, research insight, or trade thesis. |
| `raw_content` | `VARCHAR` | No | Scraped or submitted article body text (defaults to empty string). |
| `pillar` | `VARCHAR` | No | Thematic pillar (`REGULATORY`, `SECURITY_EXPLOIT`, `INSTITUTIONAL`, `MACRO_LIQUIDITY`, `USER_THESIS`). |
| `sentiment_bias` | `DOUBLE` | No | Directional sentiment bias score bounded in `[-1.0, +1.0]` (Bearish < 0, Bullish > 0). |
| `confidence_score` | `DOUBLE` | No | Subjective user confidence score bounded in `[0.0, 1.0]`. |
| `tags` | `VARCHAR` | No | Comma-separated categorization tags. |
| `created_at_utc` | `TIMESTAMPTZ` | No | UTC timestamp when intelligence was submitted and persisted. |
| `is_active` | `BOOLEAN` | No | Active flag indicating if note contributes to current committee deliberations (defaults to `TRUE`). |

### 14.2 investment_committee_memos

#### Description
Maintains daily institutional Investment Memorandums generated by the multi-persona AI Investment Committee, capturing consensus scores, individual theses, dissenting opinions, proposed allocations, and symbolic RiskGuard invariant clamping verification.

- **Layer**: Decision & Audit Serving Layer
- **Physical Location**: `data/state/platform.duckdb`
- **Primary Key**: `memo_id`

| Column Name | Data Type | Nullable | Description & Business Rules |
| :--- | :--- | :--- | :--- |
| `memo_id` | `VARCHAR` | No | Unique memorandum identifier (`memo_YYYYMMDD_...`). Primary key. |
| `memo_date` | `DATE` | No | Effective calendar trade date in UTC. |
| `created_at_utc` | `TIMESTAMPTZ` | No | UTC timestamp when the deliberation occurred. |
| `market_regime` | `VARCHAR` | No | Prevailing macro regime (`RISK_ON_EXPANSION`, `CAUTIOUS_BULL`, `NEUTRAL_CHOP`, etc.). |
| `composite_mni` | `DOUBLE` | No | Input Composite Macro-Narrative Index in `[-1.0, +1.0]`. |
| `consensus_score` | `DOUBLE` | No | Multi-persona weighted consensus score in `[-1.0, +1.0]`. |
| `executive_summary_id` | `VARCHAR` | No | Localized institutional executive summary commentary in Bahasa Indonesia. |
| `macro_thesis` | `VARCHAR` | No | Macro & Geopolitical Strategist persona outlook and liquidity thesis. |
| `valuation_thesis` | `VARCHAR` | No | On-Chain & Fundamental Valuation Analyst persona thesis. |
| `technical_thesis` | `VARCHAR` | No | Technical Momentum & Risk Officer persona thesis. |
| `dissenting_opinions` | `VARCHAR` | No | Contrarian perspectives, minority votes, and friction points. |
| `proposed_action` | `VARCHAR` | No | Recommended tactical stance (`ACCUMULATE`, `HOLD`, `PRESERVE_CASH`). |
| `proposed_allocation_usd` | `DOUBLE` | No | Neural unconstrained proposed capital deployment in USD ($D_{\text{proposed}}$). |
| `clamped_allocation_usd` | `DOUBLE` | No | Symbolic solver verified and clamped capital allocation in USD ($D_{\text{clamped}}$). |
| `allocation_clamped` | `BOOLEAN` | No | Boolean flag indicating whether symbolic invariants clamped the neural proposal. |
| `clamping_reason` | `VARCHAR` | Yes | Mathematical invariant justification if clamped (`NULL` if unclamped). |
| `risk_guard_passed` | `BOOLEAN` | No | Pre-Trade RiskGuard execution gatekeeper pass status (`TRUE` if order permitted). |
| `memo_markdown` | `VARCHAR` | No | Full rendered institutional investment memorandum formatted in Markdown. |

### 14.3 investment_committee_votes

#### Description
Stores granular voting records cast by each committee persona (`MacroStrategist`, `ValuationAnalyst`, `RiskOfficer`) for every deliberation session.

- **Layer**: Deliberation Audit Layer
- **Physical Location**: `data/state/platform.duckdb`
- **Primary Key**: `vote_id`

| Column Name | Data Type | Nullable | Description & Business Rules |
| :--- | :--- | :--- | :--- |
| `vote_id` | `VARCHAR` | No | Unique vote identifier (`vote_...`). Primary key. |
| `memo_id` | `VARCHAR` | No | Foreign key referencing parent `investment_committee_memos.memo_id`. |
| `persona` | `VARCHAR` | No | Committee persona name (`MacroStrategist`, `ValuationAnalyst`, `RiskOfficer`). |
| `stance` | `VARCHAR` | No | Persona tactical recommendation stance (`ACCUMULATE`, `NEUTRAL_HOLD`, `DEFENSIVE_HOLD`). |
| `target_allocation_usd` | `DOUBLE` | No | Persona proposed capital allocation target in USD. |
| `confidence` | `DOUBLE` | No | Persona confidence rating bounded in `[0.0, 1.0]`. |
| `rationale` | `VARCHAR` | No | Specific qualitative and quantitative justification for persona vote. |
| `voted_at_utc` | `TIMESTAMPTZ` | No | UTC timestamp when vote was recorded. |

### 14.4 mart_committee_deliberation_daily (Analytical View)

#### Description
Unified conformed analytical serving view joining daily market prices, on-chain valuation multiples, 3-tier macro narrative intelligence, committee deliberation outcomes, and active user alpha count.

- **Layer**: Curated Analytical Mart View
- **Physical Location**: `data/state/platform.duckdb`
- **Base Tables/Views**: `mart_macro_narrative_daily`, `investment_committee_memos`, `user_market_intelligence`
...
| `mvrv_ratio` | `DOUBLE` | Yes | On-chain Market Value to Realized Value ratio. |
| `fng_value` | `INTEGER` | Yes | Daily Crypto Fear & Greed Index score (0 to 100). |
| `composite_mni` | `DOUBLE` | No | Composite Macro-Narrative Index in `[-1.0, +1.0]`. |
| `macro_regime` | `VARCHAR` | No | Macro-Narrative regime classification. |
| `black_swan_flag` | `BOOLEAN` | No | Emergency halt sentinel indicator. |
| `memo_id` | `VARCHAR` | Yes | Associated daily investment committee memo ID (or `NULL` if not deliberated). |
| `consensus_score` | `DOUBLE` | Yes | Multi-persona weighted consensus score. |
| `proposed_action` | `VARCHAR` | Yes | Recommended committee action. |
| `proposed_allocation_usd` | `DOUBLE` | Yes | Neural unconstrained proposed allocation in USD. |
| `clamped_allocation_usd` | `DOUBLE` | Yes | Symbolic solver verified allocation in USD. |
| `allocation_clamped` | `BOOLEAN` | Yes | Boolean flag indicating if allocation was clamped. |
| `clamping_reason` | `VARCHAR` | Yes | Symbolic invariant clamping explanation. |
...
## 15. Type System & Serialization Conventions
